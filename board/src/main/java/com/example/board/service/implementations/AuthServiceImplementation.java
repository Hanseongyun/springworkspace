...
@RequiredArgsConstructor    // (final) 생성자를 통한 의존성주입 2
public class AuthServiceImplementation implements AuthService{

    private final UserRepository userRepository;        // userRepository 를 받아옴 1
    ...
    public ResponseEntity<ResponseDto> signUp(SignUpRequestDto dto) {
        ...
        try {
            ...
            // 2-1. 1에 대한 조회결과가 true이면 중복된 닉네임 응답 처리
            if (isExistNickname) return ResponseDto.duplicateNickname();

            // 3. 입력받은 휴대전화번호가 유저 테이블에 존재하는지 조회
            // telNumber -> (true, false)
            String telNumber = dto.getTelNumber();
            boolean isExistTellNumber = userRepository.existsBytelNumber(telNumber);

            // 3-1. 1에 대한 조회결과가 true이면 중복된 휴대전화번호 응답 처리
            if (isExistTellNumber) return ResponseDto.duplicateTelNumber();
