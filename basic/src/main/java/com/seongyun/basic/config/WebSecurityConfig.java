...
import com.seongyun.basic.provider.OAuth2SuccessHandler;
import com.seongyun.basic.service.implement.OAuth2UserServiceImplement;
...
@Configuration
// @Configurable :
// - 생성자나 '매서드'가 호출시에 Spring bean 등록을 자동화(제어 역전)하는 어노테이션
@Configurable
// @EnableWebSecurity :
...
public class WebSecurityConfig {
    ...
    // @Bean :
    // - Spring bean으로 등록하는 어노테이션         스프링이 알아서 제어의 역전시켜서 인스턴스를 생성?
    // - @Component를 사용하지 못할 때 사용
    // - @Autowired의 목적이 아닐때 (인스턴트 생성이 목적이 아닐적에?)
    @Bean
    protected SecurityFilterChain configure(HttpSecurity security) throws Exception {
        ...
        security
        // .httpBasic : basic authentication 미사용 지정
        .httpBasic(HttpBasicConfigurer::disable)        // 매개변수로 disable 매서드를 전달
        // session : 
        ...
        // -공격자가 웹 브라우저에 악성 스크립트를 작성하여 실행시키는 공경
