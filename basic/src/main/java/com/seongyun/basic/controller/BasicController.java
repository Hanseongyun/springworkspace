...
// HTTP GET localhost:4000/main/**  요청에 대해서는 이 컨트롤러로 사용하겠다
@RequestMapping("/main")
// @AllArgsConstructor
...
public class BasicController {
    // private Basicservice service;           // 생성자를 통한 의존성 주입
    ...
    // - spring framework에서 의존성 주입은 제어의 역전 (IOC)를 통해서 spring framwork가 진행 (@Component를 사용해서 Spring Bean으로 등록된 Class에 한해서)
    ...
    // HTTP GET localhost:4000/main/    요청을 의미 
    ...
    // @GetMapping() : RequestMapping 기능을 GET Method에 한정시킨것 (가독성 + 안정성)
    @GetMapping("/apple")
    ...
    // @PostMapping() : RequestMapping 기능을 Post Method에 한정시킨것 (가독성 + 안정성)
    @PostMapping("/apple")
    public String postApple() {
        return "Post Mapping으로 만든 메서드";
    }

    // HTTP PUT Method : 클라이언트가 서버에 있는 리소스 전체를 수정하고 싶을때 사용하는 메서드
    // @PutMapping() : RequestMapping 기능을 Post Method에 한정시킨것 (가독성 + 안정성)
    @PutMapping("/apple")
    ...
    // HTTP DELETE Method : 클라이언트가 서버에 있는 리소스를 삭제하고 싶을때 사용하는 메서드
    // @DeleteMapping() : RequestMapping 기능을 Delete Method에 한정시킨것 (가독성 + 안정성)
    @DeleteMapping("/apple")
