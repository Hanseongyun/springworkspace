...
// Request Body로 받는 데이터에 대한 유효성 검사를 validation 의존성을 추가해야 함
public class SampleDto {
    
    // 해당 속성을 null이 올 수 없도록 지정
    @NotNull 
    private String notNull;          // 캡슐화 - private
    ...
    // @NotEmpty @NotBlank         
    private Integer requiredInt;        // int일 경우 0이떠서 못잡아냄 > Integer

    // 문자열일 경우 문자열 길이의 최소 최대를 지정(null 값이면 검사 안함) @Length에는 @NotNull까지 같이 걸어줘야 안정적임
    @Length(min=5, max=10)
    @NotNull
